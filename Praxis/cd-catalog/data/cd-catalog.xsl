...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <!--<xsl:param
  name="suchpfad" select="CATALOG/CD[starts-with(COUNTRY, $country)]" />-->
  <xsl:param name="suchpfad"
    select="CATALOG/CD[
  starts-with(COUNTRY, $country) 
  and contains (
    translate( ., 'ABCDEFGHIJKLMNOPQRSTUVWXYZÄÖÜ', 'abcdefghijklmnopqrstuvwxyzäöü'),
    translate($stichwort, 'ABCDEFGHIJKLMNOPQRSTUVWXYZÄÖÜ', 'abcdefghijklmnopqrstuvwxyzäöü')
  )
]" />
    ...
    <html lang="de">
      ...
      <body>
        ...
        <form action="index.php" method="get">
          <!-- Radiobutton für Anzeige aller Länder -->
          <input type="radio" name="country" id="country0" value="">
            <xsl:if test="$country=''">
              <xsl:attribute name="checked">checked</xsl:attribute>
            </xsl:if>
          </input>
          <label for="country0">
            <img src="img/worldwide.svg" alt="All Länder" />
          </label>
          <!-- Ein Radiobutton für jedes unterschiedliche Land -->
          <xsl:for-each select="CATALOG/CD">
            <xsl:sort select="COUNTRY" order="ascending" />
            <xsl:if
              test="not(preceding-sibling::CD/COUNTRY=COUNTRY)">
              <input type="radio" name="country" id="country{position()}" value="{COUNTRY}">
          ...
          <input type="text" id="stichwort" name="stichwort" value="{$stichwort}"></input>
          <button type="submit">Suchen</button>
          <button type="reset">Formuler zurücksetzen</button>
        </form>
        <!-- Hinweis fürden  Nutzer-->
        <p>Ich habe
          <strong>
            <xsl:value-of select="$anzTreffer" />
          </strong> CDs.
        </p>

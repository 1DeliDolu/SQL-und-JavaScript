(() => {
    ...
    const speicherListe = (evt) => {
        ...
        if (
            ...
            (alteListe && !confirm("Vorhandene Liste im Speicher überschreiben?"))
        ) {
            return;
        }
    ...
    const ladeListe = (evt) => {
        ...
        if (
            ...
            (EINKAUFSLISTE.hasChildNodes() &&
                !confirm(
                    "Möchten Sie die angezeigte Liste mit Daten aus dem Speicher überschreiben?"
                ))
        ) {
            return;
        }
